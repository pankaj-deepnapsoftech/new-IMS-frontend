          ...
          <TableContainer overflowY="auto" borderRadius="md" className="mx-3 bg-[#ffffff26]">
            <Table variant="unstyled" {...getTableProps()}>
              <Thead className="text-sm font-semibold bg-[#ffffff26]">
                {headerGroups.map((hg) => (
                  <Tr {...hg.getHeaderGroupProps()} border="none">
                      ...
                      <Td border="none" className="flex gap-x-2 items-center">
                        {openEmployeeDetailsDrawerHandler && (
                          <MdOutlineVisibility
                            ...
                            size={16}
                            onClick={() => openEmployeeDetailsDrawerHandler(row.original?._id)}
                          />
                        )}
                        {openUpdateEmployeeDrawerHandler && (
                          <MdEdit
                            className="hover:scale-110"
                            size={16}
                            onClick={() => openUpdateEmployeeDrawerHandler(row.original?._id)}
                          />
                        )}
                        {deleteEmployeeHandler && (
                          <MdDeleteOutline
                            className="hover:scale-110"
                            size={16}
                            onClick={() => deleteEmployeeHandler(row.original?._id)}
                          />
                        )}
                        {approveEmployeeHandler && (
                          <FcApproval
                            className="hover:scale-110"
                            size={16}
                            onClick={() => approveEmployeeHandler(row.original?._id)}
                          />
                        )}
                      </Td>
                    </Tr>
