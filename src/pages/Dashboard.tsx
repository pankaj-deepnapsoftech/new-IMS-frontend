...
const Analytics: React.FC = () => {
  ...
  const fetchSalesDeliveredData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        console.log("Sales-Delivered API Response:", data); // Debug log
        if (data.success) {
  ...
  const fetchMachineStatus = async () => {
    try {
      ...
      const response = await fetch(
        "http://localhost:8085/api/dashboard/machine-status",
        {
          headers: {
            Authorization: `Bearer ${cookies?.access_token}`,
            "Content-Type": "application/json",
  ...
  const kpiCards = [
    ...
    },
    {
      ...
      value: isLoadingStats
        ? "..."
        : statsData?.verified_employees?.total?.toString() || "0",
      change: isLoadingStats
        ? "..."
        : Math.abs(
